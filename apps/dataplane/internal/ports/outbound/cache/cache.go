// Package cache is this application's boundary for ephemeral, TTL-scoped
// state.
//
...
// implementation lives in internal/adapters/outbound/valkey — the one
// auditable site of a store-client import — and nothing here knows which
// server, client library, or protocol version serves the bytes. The local
// deployment record (deploy/redis/README.md) states what may live in that
// store and what may never.
//
// This port belongs to the Data Plane; the Control Plane has its own copy in
// its own module, with its own keyspace prefix. An entry here is never
// authoritative even for the runtime — it is a cache, and losing it costs a
// recomputation and nothing else, which is exactly why no admission decision
// may rest on one.
//
