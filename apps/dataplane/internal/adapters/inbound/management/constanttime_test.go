...
// parseSource reads one of this package's source files, for the tests in this
// package that assert something about the shape of the code rather than about
// what it does. protocol_test.go's failure-vocabulary census is the other one.
//
// The FileSet comes back with the file because a failure has to name a line: an
// assertion of the form "no file but errors.go builds a failure" is only useful
// to whoever has to fix it if it says which file and where.
func parseSource(t *testing.T, name string) (*ast.File, *token.FileSet) {
	t.Helper()
