...
// repeating here that the identifier is caller-supplied input: the character
// set and length bound below are the only reason a value that arrives from
// outside this process is safe to put in a response header, a log line and an
...
func newRequestID() string {
	...
	if _, err := rand.Read(bytes); err != nil {
		...
		panic(serviceName + " management request ID entropy unavailable")
	}
...
// distinguishes a management handler's context from an unrelated background
