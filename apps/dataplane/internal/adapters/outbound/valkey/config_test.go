...
func TestFromEnv(t *testing.T) {
	t.Setenv("VALKEY_ADDRESS", "redis.example.test:6380")
	t.Setenv("VALKEY_USERNAME", "gateway")
	t.Setenv("VALKEY_PASSWORD", "not-for-logs")
	t.Setenv("VALKEY_DATABASE", "3")
	t.Setenv("VALKEY_DIAL_TIMEOUT", "2s")
	t.Setenv("VALKEY_CONN_TIMEOUT", "4s")
	t.Setenv("VALKEY_PIPELINE_MULTIPLEX", "1")
	t.Setenv("VALKEY_BLOCKING_POOL_SIZE", "2")
...
func TestFromEnvDefaults(t *testing.T) {
	for _, name := range []string{
		...
		"VALKEY_DIAL_TIMEOUT",
		"VALKEY_CONN_TIMEOUT",
		"VALKEY_PIPELINE_MULTIPLEX",
		"VALKEY_BLOCKING_POOL_SIZE",
	} {
		t.Setenv(name, "")
	}
...
func TestFromEnvRejectsInvalidValues(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			key:   "VALKEY_DATABASE",
			value: "one",
			want:  "VALKEY_DATABASE must be an integer",
		},
		{
			name:  "rejects a malformed dial timeout",
			key:   "VALKEY_DIAL_TIMEOUT",
			value: "soon",
			want:  "VALKEY_DIAL_TIMEOUT must be a duration",
		},
		{
			name:  "rejects a malformed connection timeout",
			key:   "VALKEY_CONN_TIMEOUT",
			value: "later",
			want:  "VALKEY_CONN_TIMEOUT must be a duration",
		},
		{
			name:  "rejects a non-integer connection bound",
			key:   "VALKEY_PIPELINE_MULTIPLEX",
			value: "a few",
			want:  "VALKEY_PIPELINE_MULTIPLEX must be an integer",
		},
		{
			name:  "rejects a non-integer blocking connection bound",
			key:   "VALKEY_BLOCKING_POOL_SIZE",
			value: "some",
			want:  "VALKEY_BLOCKING_POOL_SIZE must be an integer",
		},
	}
