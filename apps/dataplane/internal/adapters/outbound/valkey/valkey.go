// Package valkey owns the dataplane's connection lifecycle to a
...
//
// Nothing wires this into the runtime yet, by design: the package is the
...
//
// The Control Plane has its own copy of this adapter in its own module, with
// its own keyspace prefix: the two planes may share one server, and a key
// written by one must never be readable as the other's (ADR 0006 §7).
package valkey
