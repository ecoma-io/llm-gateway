...
// TestTheControlPlaneHasExactlyOneCrossPlaneSeam is the Console API's own
// structural claim: the Control Plane reaches the Data Plane through one
// outbound port and no other mechanism (ADR 0006 §5). A second port named for
// the runtime, or a port named for the management API the runtime is not,
// would be a second seam — and the seam is where the plane boundary is
// supposed to be legible, which it is not if there are several.
//
