...
type Config struct {
	...
	// Database is the logical SELECT index, 0 by default. One index may serve
	// more than one application: what keeps two planes' keys apart is the
	// keyspace prefix each application owns, not the index it selects.
	Database int
...
// empty VALKEY_PASSWORD is a legitimate "no password", not a fallback
// candidate.
//
...
func FromEnv() (Config, error) {
	database, err := envInt("VALKEY_DATABASE", 0)
	if err != nil {
		return Config{}, err
	}
	dialTimeout, err := envDuration("VALKEY_DIAL_TIMEOUT", defaultDialTimeout)
	if err != nil {
		return Config{}, err
	}
	connTimeout, err := envDuration("VALKEY_CONN_TIMEOUT", defaultConnTimeout)
	if err != nil {
		return Config{}, err
	}
	multiplex, err := envInt("VALKEY_PIPELINE_MULTIPLEX", defaultPipelineMultiplex)
	if err != nil {
		return Config{}, err
	}
	blockingPoolSize, err := envInt("VALKEY_BLOCKING_POOL_SIZE", defaultBlockingPoolSize)
	if err != nil {
		return Config{}, err
	}
	cfg := Config{
		Address:           envString("VALKEY_ADDRESS", defaultAddress),
	...
	if err := cfg.Validate(); err != nil {
		return Config{}, fmt.Errorf("valkey: environment configuration: %w", err)
	}
	return cfg, nil
}
...
// `slog.Info("valkey", "config", cfg)` redacts by construction — the requirement
...
func envInt(name string, fallback int) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("valkey: %s must be an integer, got %q", name, v)
	}
	return n, nil
}
...
func envDuration(name string, fallback time.Duration) (time.Duration, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("valkey: %s must be a duration like 5s, got %q", name, v)
	}
	return d, nil
}
