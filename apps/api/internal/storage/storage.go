...
// lives in internal/storage/postgres; which driver that adapter finally
// opens is decided where the wiring is, not here, and never by the code that
// asks for a Store.
//
...
// Querier is the query surface the storage layer grants for running SQL, and
// the only one a repository is given: three methods, no begin, no commit, no
// pool to reach for. Which handle answers behind the interface is the
// storage layer's decision — inside a unit of work the Querier IS that
