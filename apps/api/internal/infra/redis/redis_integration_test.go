...
// Run:
//
//	REDIS_ADDRESS=127.0.0.1:6379 go test -tags=integration ./internal/infra/redis
//
// REDIS_USERNAME and REDIS_PASSWORD are respected too, so a deployment test
// can prove ACL configuration without changing this file.
func integrationConfig(t *testing.T) Config {
	t.Helper()
	address := os.Getenv("REDIS_ADDRESS")
	if address == "" {
		t.Fatal("REDIS_ADDRESS is required for integration tests; start deploy/redis first")
	}
	return Config{
		Address:           address,
