	...
	tests := []struct {
		...
		},
		{
			...
			env: map[string]string{
				...
				"GATEWAY_READ_HEADER_TIMEOUT": "3s",
			},
			want: Config{
				Addr:              "127.0.0.1:9090",
		...
		},
		{
			...
			wantErr: "GATEWAY_ADDR must not be empty",
		},
		{
			...
			wantErr: "GATEWAY_ADDR must be a host:port address",
		},
		{
			...
			wantErr: "GATEWAY_ADDR must contain a numeric port",
		},
		{
			...
			wantErr: "GATEWAY_SHUTDOWN_TIMEOUT must not be empty",
		},
		{
			...
			wantErr: "GATEWAY_SHUTDOWN_TIMEOUT must be greater than zero",
		},
		{
			...
			wantErr: "GATEWAY_READ_HEADER_TIMEOUT must be greater than zero",
		},
		{
			name: "rejects a malformed duration",
			env: map[string]string{
				"GATEWAY_READ_HEADER_TIMEOUT": "soon",
			},
			wantErr: "GATEWAY_READ_HEADER_TIMEOUT must be a Go duration",
		},
	}
