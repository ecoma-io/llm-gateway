// Package application is the gateway's use-case boundary.
//
...
// New constructs the gateway application around the build version supplied by
// cmd/gateway. The command's package-level version variable remains the one
// ldflags source; this package receives that value, never recreates it.
