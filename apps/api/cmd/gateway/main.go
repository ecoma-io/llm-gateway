...
//
// Everything HTTP lives in internal/http so it can be tested through httptest
// without a process; what remains here is the version stamp, the listener, and
...
import (
	...
	"github.com/ecoma-io/llm-gateway/apps/api/internal/application"
	"github.com/ecoma-io/llm-gateway/apps/api/internal/config"
	gatewayhttp "github.com/ecoma-io/llm-gateway/apps/api/internal/http"
)
...
func main() {
	...
	if err != nil {
		log.Printf("gateway configuration: %v", err)
		os.Exit(1)
	}
	...
	if err != nil {
		log.Printf("gateway listen: %v", err)
		os.Exit(1)
	}
	...
	server := &stdhttp.Server{
		Handler: gatewayhttp.New(application.New(version)),
	...
	log.Printf("gateway %s listening on %s", version, listener.Addr())
