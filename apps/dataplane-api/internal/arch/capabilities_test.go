...
//     It is empty for the two rules that allow nobody, and the check at the
//     bottom of this file holds that emptiness to the rule's own shape.
//   - refused names a package the rule must not cover, chosen where possible to
//     be the crossing this boundary exists for rather than any package that
...
// imports_test.go — so the two rules that allow nobody contribute a case each,
// and the one rule that forbids three prefixes contributes three. The two
// tables are held together by
...
var capabilities = []capability{
	{
		...
		// A cache client in the facade: the transport acquiring state of its
		// own, from a package the rule allows nobody to be.
		refused: "internal/application",
	},
	{
		prefix: "net/http",
		// An application that speaks HTTP is an application whose use-cases
	...
	{
		prefix: "database/sql",
		// The facade opening a connection of its own to the Data Plane's
		// database rather than reading it over the seam.
		refused: "internal/adapters/outbound/dataplane",
	},
	{
		prefix:  "github.com/jackc/pgx",
		imports: "/v5",
		// The same crossing by the driver rather than by the primitive, which
		// is why `database/sql` alone would not have been the rule. The path
		// below the prefix is the driver's real import path, so this case also
		// pins that the rule matches a prefix rather than the exact module.
		refused: "internal/adapters/outbound/dataplane",
	},
	{
		prefix: "github.com/lib/pq",
		// And the third spelling of it, because a rule naming the drivers that
		// exist today is a rule against those drivers and nothing else.
		refused: "internal/adapters/outbound/dataplane",
	},
	{
		prefix:  "<module>/internal/ports",
		imports: "/outbound/dataplane",
		// The port named by a package that has no business in the seam: the
		// configuration loader, which would make the port a configuration
		// concern instead of an application one.
		permitted: "internal/adapters/inbound/http",
