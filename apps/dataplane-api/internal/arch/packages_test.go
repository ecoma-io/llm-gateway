...
func TestEveryPackageLivesUnderASanctionedRoot(t *testing.T) {
	// The list is shorter than its siblings' by exactly the two trees this
	// application must not have. `internal/ports/outbound` and
	// `internal/adapters/outbound` are absent rather than merely unused: a
	// directory a package may live in is a directory a package will live in,
	// and this application's whole structural claim is that it owns no data.
...
// transport. It holds no state of its own, so it has no persistence port, no
// cache port and no adapter tree to implement either — everything it will
// eventually answer comes from the Data Plane over a seam that does not exist
// yet, which §9 records as the split's one open question.
//
// A package appearing under either tree is the boundary being crossed in the
// most ordinary way there is: someone needs a row, the shortest path to one is
// a query, and the management API quietly becomes a second owner of Data Plane
// data.
func TestTheManagementTransportHoldsNoState(t *testing.T) {
	for _, dir := range packageDirs(t) {
		for _, root := range []string{"internal/ports", "internal/adapters/outbound"} {
			if under(dir, root) {
				t.Errorf("%s holds Go files: this application owns no data and so has no %s tree — its answers come from the Data Plane it manages (ADR 0006 §9, §11)", dir, root)
			}
		}
	}
}
