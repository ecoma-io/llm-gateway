...
// (vite.config.ts `server.proxy`), so `pnpm dev:console-api` + `pnpm
// dev:console` need no configuration. `VITE_API_BASE_URL` stays as the
// optional seam for pointing the console at a Control Plane API on another
// origin; there is no such seam for the runtime, deliberately.
import {
  client,
  getHealth,
  getReadiness,
