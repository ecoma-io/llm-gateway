...
`idempotency_conflict`; steps 5–7 are the admission transaction, which reads
...
happened, never a second execution of it.
...
| 3   | Request scoping        | The request body is read once, under a size cap, as **raw bytes**; its SHA-256 digest (lowercase hex) is the request's identity for every later comparison. The `Idempotency-Key` header is required (ADR 0004) and is itself grammar-checked (length and printable-ASCII, no whitespace), and the model field is checked for a bounded, well-formed value.                                                                                                                                                                                                                          | A malformed key, a body past the cap or a body that is not JSON each reject `400 invalid_request` in a transaction that writes a `rejected` row and nothing else.                                                                                                                                                                                                                                                                 |
| 4   | Idempotency intake     | One pre-transaction read of `request_intake` keyed `(account_id, idempotency_key)`, **before alias resolution**: a row whose digest matches is a replay — the original request id and its terminal status are re-answered with `replayed=true`; the original body is not replayed and the request is never re-executed (ADR 0004). A row whose digest differs is an explicit conflict.                                                                                                                                                                                               | `409 idempotency_conflict` (different digest, no `Retry-After`, no new row — the original request's row answers) or `409 request_in_progress` (same digest, original admission still open, with a fixed short `Retry-After` — no row is written).                                                                                                                                                                                 |
| 5   | Model alias resolution | Inside the admission transaction: alias lookup in the runtime's own alias and candidate rows, one consistent candidate snapshot (ADR 0001, rule 3); the alias must be `active`. The request's canonical `max_output_tokens` is validated (present, positive, within the alias limit) and input tokens are counted with the gateway's canonical tokenizer. The admission-time price revision is resolved in the same transaction, and the hold is sized: `ceil((T_in·p_in + T_out·p_out) / 1_000_000)` in integer minor units per 1M tokens, one ceiling over the summed raw product. | `404 model_not_found` (unknown or inactive alias) or `400 invalid_request` (the output bound, the `model` param) — a `rejected` request row is written with the fields known so far. A missing price is a `500` that writes **nothing**: an unpriced alias is a configuration state the runtime refuses, never a price of zero. A hold above the alias's `reservation_cap` is `400 invalid_request` before any capacity is taken. |
| 6   | Admission decision     | The ADR 0003 waterfall runs against the runtime's **quota projections** at `transaction_timestamp()` — the database's own clock, never a node's. Matching grants are walked in the domain's total order and drawn by conditional update; if a drawdown does not take all the capacity asked of it, the shortfall is given back unconditionally before the answer is decided. The walk is re-read after any failed take (at most three passes) so a contender's win is a re-decide, never a stale verdict.                                                                            | Zero eligible capacity at all → every giveback, then `403 no_access`. Capacity present but the whole hold not secured → every giveback, then `429 insufficient_quota` (the reason the schema calls `insufficient_entitlement`, with the PAYG verdict — see [commerce](commerce.md)). Nothing is left drawn behind either refusal: the walk is all-or-nothing against the hold.                                                    |
...
| 11  | Usage capture          | From the committed attempt, at the **customer-visible delivery boundary**: provider-reported usage (`reported`), else forwarded-delta counting (`gateway_observed`), else the held amount (`reservation_floor`); ADR 0004.                                                                                                                                                                                                                                                                                                                                                           | Unknowable usage still settles conservatively; a fact is never invented and never exceeds the hold.                                                                                                                                                                                                                                                                                                                               |
