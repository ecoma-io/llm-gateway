...
| Primary keys       | UUID version 7 for any row whose identity crosses a plane or reaches a client; deviations require a stated reason                                                                                                                                                                                          |
| Timestamps         | `timestamptz`, stored UTC, suffixed `_at` — except where an ADR has already named the column (`effective_from`, the `period_start`/`period_end` bounds), whose names stand                                                                                                                                 |
| Nullability        | `NOT NULL` by default; a nullable column states its reason                                                                                                                                                                                                                                                 |
...
| JSONB              | opaque provider telemetry only — never a load-bearing, queryable field; everything the model requires to be queryable is a column                                                                                                                                                                          |
...
| Constraint names   | `<table>_<columns>_idx`, `<table>_<columns>_key`, `<table>_<column>_fkey` — PostgreSQL's own defaults, kept rather than overridden                                                                                                                                                                         |
