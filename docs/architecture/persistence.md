...
| Primary keys       | UUID version 7 for any row whose identity crosses a plane or reaches a client; deviations require a stated reason. The stated deviations: the Control Plane's API-key id is UUIDv4, because the id is the token's prefix material and the token grammar pins the v4 form (`identity`'s `validateUUIDForm`; the `api_keys_prefix_shape` CHECK enforces it in the schema); identity's account and user ids are UUIDv4 because the identity foundation predates this rule and nothing consumes their order — they stay as minted. From the commerce foundation on, plane-crossing and client-facing ids mint v7 — the waterfall's last tie-break reads entitlement ids as time order — and the schema pins the form where an id names another context's row (`account_payg_funding_bucket_uuid_v7`) |
