...
| `projection_changes`, `projection_api_keys`, `projection_accounts`, `projection_revision` | Control Plane | Projected from here: the credential projection's producer half (ADR 0007) — the durable change log, the materialized current state and the gapless counter, each written in the same transaction as the authority write above. This is the one named place the secret's digest transits this plane's database; the ownership record above stays digest-free.       |
| API-key authentication records                                                            | Data Plane    | Observed here: the secret's hash, the lookup, the runtime's own active/revoked state and scopes. Authenticating on the hot path is the runtime's job, so the row it authenticates against is the runtime's. Fed by the credential projection (ADR 0007), whose consumer position — `projection_state`, one singleton — is the runtime's own half of that pipeline. |
...
| `requests`, `request_attempts`, `usage_events`                                            | Data Plane    | Observed here: what actually happened on the wire. The high-volume time-series facts (ADR 0005) — landed as plain tables, not the hypertables the ADR first placed here; its B7 amendment records why.                                                                                                                                                             |
...
(ADR 0006, section 8). The projection refines the first half without breaking
it: the digest now transits the Control Plane's own database, but only inside
the projection pipeline's own tables ([ADR 0007](../adr/0007-control-to-data-projection.md)),
which are the one named place it is allowed to exist on this side — the
ownership record keeps no digest column, and the session surface is no closer
to it than before.
...
| Transport       | a management call the Data Plane can refuse, or — for the key credential — the projection's push with replay ([ADR 0007](../adr/0007-control-to-data-projection.md)); the same chain below carries both | a durable pull the consumer drives; the Data Plane never pushes and never calls back                        |
| Cursor          | none for a command — it names its entity; the projection holds a revision position on both sides, the Data Plane's applied revision and this plane's log head                                           | an opaque position in the Data Plane's fact order, issued by the Data Plane and stored by the consumer only |
| Idempotency key | the entity's own identifier; the projection's full-state entries and position make a redelivery a no-op answer                                                                                          | `request_id`                                                                                                |
...
command the Data Plane applies, a projection message it applies against its
position, or a page of facts it hands back. The protocols the last two keep —
the projection's revisions, snapshot and replay one way, the fact feed's
cursor, replay and reconciliation the other — are
